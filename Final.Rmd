...
4) Next we will create an aggressive portfolio which increases our return as well as our risk. After trying several combinations, below is the weight we considered for this portfolio:
...
Next, run a function _ClusterTweeters_. That function, given a K and a data frame will return the CH score of all that model. _withoutChatter_ is a matrix with the CH score and SSE(tot) of all the models tested between k = 1 and k = 15. This step included **%dopar%** to tell the foreach loop to compute the results in parallel.
```{r}
